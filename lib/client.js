...
class Client extends EventEmitter {
  ...
  constructor(options) {
    ...
    this._settings = {
      ...
      broadcastSocket: ('broadcastSocket' in options) ? options.broadcastSocket : true
    };
    ...
    this._transport = this._settings.transport || new baTransport({
      ...
      broadcastSocket: this._settings.broadcastSocket
    });
  ...
  _addCallback(id, callback) {
    const timeout_ms = Math.floor(this._settings.apduTimeout * (1 + 0.25*Math.random()));
